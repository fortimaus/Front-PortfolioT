<template>
  <div class="modal fade" id="updateAchModal" tabindex="-1" aria-hidden="true">
...
export default {
  ...
  mounted() {
    this.modalInstance = new Modal(document.getElementById('updateAchModal'))
  },
  methods: {
    async show(id) {
      this.resetForm()
      this.id = id
      await AchievementService.get(id)
        .then(response=>{
    ...
    async submitForm() {
      ...
      let imagePreview = null;
        ...
        console.log(data)
        await AchievementService.update(data)
        .then(response=>
        { 
          console.log(response)
          this.hide()
          this.$emit('ach-update')
        })
